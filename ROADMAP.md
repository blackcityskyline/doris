...
**Second thing to check, at runtime rather than compile time:** `TorrentInfo` in
`src/torrserver/api.rs` (Phase 7) assumes TorrServer's `/torrents` JSON response
uses capitalized Go field names (`Hash`, `Name`, `TorrentSize`, `DownloadSpeed`, ...)
with no `json` tags, based on reading YouROK/TorrServer's source on GitHub rather
than a live server. Every field has `#[serde(default)]`, so a wrong/renamed field
degrades to a zero value instead of breaking parsing — but if the Torrent panel
shows all zeros against a TorrServer that IS reachable and DOES have an active
torrent, this field-name mapping is the first place to check.
`tests/torrserver_tests.rs` documents the assumed shape.
...
| 7 | `torrent::Manager` polling loop (fixes B3) + pause/resume/drop + TorrServer enable/disable-with-sudo-prompt in Options | 🔶 `torrent::Manager` polling + pause/resume/remove done (see below). TorrServer enable/disable-with-sudo-prompt is still ⏳ planned -- deliberately: it needs to know how *this* TorrServer instance is actually being run (systemd service? standalone binary Doris itself spawned? Docker?), which isn't discoverable from the code alone. Options -> streaming -> TorrServer currently offers a real reachability check instead (`TorrServer::is_reachable()`) rather than a guessed-at start/stop flow that might not match the user's actual setup. |
| 8 | Torrent panel dot/braille progress graph (btop-style) | ✅ done. New `ui/widgets/graph.rs::render_sparkline()` (pure function, unit-tested in `tests/graph_tests.rs`; moved from `ui/graph.rs` into `ui/widgets/` in Phase 10) replaces the static `[####    ] 0%` bar with a real history sparkline, consuming the `graph_symbol` setting from Phase 5 (braille/block/dot) which was persisted-but-inert until now. `ui::App` tracks a capped `progress_history` fed by the Phase 7 poller, cleared when the active torrent changes or is removed so graphs don't bleed between torrents. |
| 9 | Mouse: clickable keybind hints, scroll in every zone | ✅ done. New `HeaderHint`/`hint_at_column()` makes every top-bar hint ("s: search", "S: settings", "L: log", "F: filter") a real click target dispatching the same action as its key. New shared `zone_at()` hit-test replaces the old Results-only `click_results_at`: clicking *any* zone focuses it (btop-style click-to-focus), and the scroll wheel now targets whichever zone the mouse is hovering over (previously it only ever scrolled whatever was keyboard-focused, regardless of where the mouse was) and works uniformly across all four zones (Torrent/Extra correctly no-op since they have nothing scrollable yet, rather than being silently unreachable). The Torrent panel's own "p: pause/resume d: remove" hint is clickable too, wired through the same click-handling path. `handle_mouse` is now `async` so scroll-down can share the exact same load-more-pagination logic as keyboard nav (`handle_nav_down`) instead of a separate, slightly-different code path. |
| 10 | Cleanup: drop unused `chromiumoxide`/`chromiumoxide_cdp` deps, split `ui/app.rs` into `ui/modals/*`, update `AGENTS.md` to match the new module layout | 🔶 Done: dropped `chromiumoxide`/`chromiumoxide_cdp` (confirmed zero references before removing); deleted `ui/draw.rs` (confirmed zero references -- dead since before this refactor started, not something Phases 1-9 orphaned); moved `ui/graph.rs` → `ui/widgets/graph.rs`; `AGENTS.md` rewritten to match every phase above (module tree, zone descriptions, key bindings, dependency list). **Not done, deliberately:** splitting `ui/app.rs` (now ~1,800 lines) into `ui/modals/{settings,login,health}.rs`. This file is exactly the kind of large, deeply interconnected module (`Modal`/`SettingsState`/`ZoneLayout` access, `self.theme` reads threaded through nearly every render function) where a mechanical split is easy to get subtly wrong -- circular-looking `use super::app::{...}` imports between the new modal modules and what's left of `app.rs`, method visibility (`pub(crate)` vs `pub`), etc. -- and where getting it wrong is expensive to debug blind, without a compiler. Splitting it is exactly the kind of change to do *with* `cargo check` in the loop, not without it. See "Suggested split" below for a concrete plan. |

**Suggested split for the remaining Phase 10 work** (for whoever picks this up with a working toolchain):
1. Move `SettingsState`/`SettingsCategory`/`SettingsItem`/`SettingsAction` + `open_settings()`/`settings_key()` + the Settings branch of `render_modal()` into `ui/modals/settings.rs`.
2. Move `LoginState`/`LoginField` + the Login branch of `handle_key`/`render_modal()` into `ui/modals/login.rs`.
3. Move `health_check()` + the HealthCheck branch of `render_modal()` into `ui/modals/health.rs`.
4. `ui/app.rs`'s `App` struct and `Modal` enum stay put (too central to move without touching everything); the three new files take `use super::app::{App, Modal, ...}` and `impl App { ... }` blocks for just their piece, same pattern this whole refactor already uses for `ui/widgets/graph.rs`.
5. Build and fix borrow-checker fallout incrementally, one moved chunk at a time, rather than moving everything in one shot.
