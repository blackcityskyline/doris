...
| 8 | Torrent panel dot/braille progress graph (btop-style) | ✅ done. New `ui/graph.rs::render_sparkline()` (pure function, unit-tested in `tests/graph_tests.rs`) replaces the static `[####    ] 0%` bar with a real history sparkline, consuming the `graph_symbol` setting from Phase 5 (braille/block/dot) which was persisted-but-inert until now. `ui::App` tracks a capped `progress_history` fed by the Phase 7 poller, cleared when the active torrent changes or is removed so graphs don't bleed between torrents. |
