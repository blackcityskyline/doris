...
| 5 | Settings rewrite: typed descriptor table, dynamic pagination (fixes B1/B2/B5), general/streaming/download categories per spec, config save-on-exit actually implemented | ✅ `general` category done. Fixes B1 (hardcoded "3/8"), B2 (page-size desync), B5 (decorative values with no backing field), and a newly-found B6 (Browser visible toggle didn't affect the actual launch). `Config` gained 14 new persisted fields + `save()`/`load()`, all wired to real behavior where one exists yet (vim_keys, disable_mouse, save_config_on_exit, theme selection+persistence, presets via a real `ZoneLayout::apply_preset`). Still honest-but-inert pending later phases: `truecolor`/`false_tty` (no color-degradation renderer yet), `rounded_corners`/`show_boxes`/`terminal_sync` (no shared block-builder to hook into across ui/app.rs's ~17 render call sites yet -- see Phase 10), `update_ms`/`graph_symbol` (nothing consumes them until Phases 7/8 exist). **Not yet done:** restructuring the `app` category into spec's `streaming`/`download` categories -- next commit. |
